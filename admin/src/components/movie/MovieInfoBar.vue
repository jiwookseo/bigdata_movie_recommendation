<template>
  <div class="data-info-bar">
    <div class="data-info" v-if="onTarget !== idx">
      {{ title }} | {{ genresStr }} | {{ rating }} | {{ viewCnt }}
    </div>
    <div class="data-edit" v-else>
      <label for="title"></label>
      <input v-model="movieTitle" id="title" type="text">
      <div class="genres">
        <button v-for="(genre, idx) of inputGenres" class="genre_button" @click="disGenre(idx)">{{ genre }}</button>
      </div>
    </div>
    <div class="data-controller">
      <i v-if="onTarget === idx" class="fas fa-check" @click="editInfo"></i> | <i class="fas fa-edit" @click="checkEdit"></i> | <i class="fas fa-trash-alt" @click="delMovie"></i>
    </div>
  </div>
</template>
